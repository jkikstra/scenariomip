...
IAM_SCENARIOS_FILE <- "scenarios_scenariomip_allmodels_2025-01-20.csv"

HISTORY.COUNTRY.AND.GLOBAL <- "cmip7_history_0002.csv"
...
scenarios_harmonization <- scenarios %>%
  ...
  # # test smaller scenario set for debugging
  # filter_model_includes("GAINS") %>% 
  # # filter_includes("Transportation") %>% filter_region_includes("Sub-S") %>% 
  # filter_scenario_includes("SSP1 - Very Low Emissions") %>% 
...
format_historical <- function(df){
  return(
    df %>% 
      filter(
        year>=1997, year<=2022 # ad-hoc; because in this version GFED doesn't go earlier than 1997, and CEDS doesn't have 2023
      )
  )
}
...
for (m in model.list.simple){
  ...
  save_ggplot(
    ...
    h = 500,
    w = 500,
    f = here("data", "data_vetting", "figures", "per-model", m, paste0("emissions_global") )
  )
}
...
for (m in model.list.simple){
  p.data.model <- scenarios_harmonization %>%
      filter(region=="World") %>% filter_includes("Total") %>% mutate(full.model.name = model) %>% 
      simplify_model_names() %>%
      filter(grepl(model, pattern=m, fixed=T)) %>% 
      iamc_region_keep_one_level(-1) %>% 
      iamc_variable_keep_one_level(-1)
  # p.data.hist <- hist.national %>% filter(region=="World") %>% filter_includes("Total") %>% 
  #       iamc_region_keep_one_level(-1) %>% 
  #       iamc_variable_keep_one_level(-1)
  ...
  save_ggplot(
    ...
    h = 500,
    w = 500,
    f = here("data", "data_vetting", "figures", "per-model", m, paste0("emissions_global_totals") )
  )
}
...
for (s in sector.list %>% pull(sector) %>% unique() ){
  p.data.model <- scenarios_harmonization %>% mutate(full.model.name = model) %>% 
      simplify_model_names() %>%
      filter(grepl(model, pattern=m, fixed=T),
             grepl(region, pattern=m, fixed=T)) %>% 
  ...
  if (nrow(p.data.model)+nrow(p.data.hist)>0){
    ...
    save_ggplot(
      ...
      h = 500,
      w = 500,
      f = here("data", "data_vetting", "figures", "per-model", m, paste0("emissions_", s) )
    )
...
dir.create(here("data", "data_vetting", "figures", "all-models-overview"))
write_delim(
  x = present.or.not,
  file = here("data", "data_vetting", "figures", "all-models-overview", "variable_reporting.csv"),
  delim = ","
)
...
for (m in present.or.not %>% pull(model) %>% unique() ){
  ...
  write_delim(
    ...
    file = here("data", "data_vetting", "figures", "per-model", m.simple, paste0("variable_reporting_",m,".csv")),
    delim = ","
  )
}
...
print(ppt, target = here("data", "data_vetting", "figures", "per-model", "compare_with_scenarioMIP_guidance.pptx"))
